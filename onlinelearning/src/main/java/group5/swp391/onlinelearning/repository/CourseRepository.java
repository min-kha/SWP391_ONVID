...
public interface CourseRepository extends JpaRepository<Course, Integer> {
    ...
    @Query(value = "SELECT Course.* FROM my_order JOIN order_detail ON my_order.id = order_detail.id and my_order.student_id = ?1 JOIN course ON course.id = order_detail.course_id group by course.id", nativeQuery = true)
